 ...
 * Where the wrapper is narrower than the table (34rem plus its 2px border), a
 * visual-only hint says the other columns are a sideways scroll away; touch
 * scrollbars are overlays, so nothing else shows it.
 */
export function SkillsMatrix({ rows, title, onSkill }: Props) {
  const id = useId();
  return (
    <section aria-labelledby={`${id}-title`} data-fit-matrix="" className="fit-block @container">
      ...
      {rows.length === 0 ? (
        ...
        <>
          <p aria-hidden="true" data-fit-matrix-hint="" className="mt-1 hidden text-xs text-text-muted print:hidden @max-[34.125rem]:block">
            Scroll sideways for projects and roles.
          </p>
          <div
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={`${r.jdTerm}-${r.siteTerm}`} data-fit-matrix-row={r.siteTerm} data-mapped={r.mappedByAi ? 'ai' : 'exact'} className="border-b border-hairline last:border-b-0 align-top">
                    {/* break-word, not anywhere: in an auto-layout table 'anywhere' counts toward the
                        column's min-content width, so the column collapses and terms split mid-word. */}
                    <th scope="row" className="px-3 py-2 font-medium text-text-primary [overflow-wrap:break-word]">
                      {r.jdTerm}
                    </th>
